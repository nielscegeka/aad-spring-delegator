...
(Indien je binnen de Free Tier wilt blijven maak je gebruik van Spring Container Apps)
Stappenplan: <br>
1A: application.properties populeren met de juiste services. <br>
1B: pom.xml populeren met de juiste services. <br>
...
5: mvn -X clean package compile jib:build -Dspring.profiles.active='profile' (Dit pusht de lokale spring image naar de ACR)  <br>
...
1: Updaten van de image: mvn -X clean package compile jib:build -Dspring.profiles.active='profile' (Dit pusht de lokale spring image naar de ACR)  <br>
2: Binnen de Azure CLI: az containerapp update --name 'app-name' --resource-group 'resource-group' --image 'acr-name'.azurecr.io/'image-name' <br>
3: In het portaal in je container > Application > Revisions & Replicas > Replicas > Je ziet je oude versie downspinnen met de nieuwe ernaast
